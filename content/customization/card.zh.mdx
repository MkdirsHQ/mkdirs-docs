...
![Item Card with Image](/images/guide-item-card-image.png)

带图标样式 ([item-card-2.tsx](https://github.com/MkdirsHQ/mkdirs-template/blob/main/src/components/item/item-card-2.tsx)):

![Item Card with Icon](/images/guide-item-card-icon.png)


`带图标样式` 是默认样式，您可以通过修改 `src/lib/constants.ts` 文件中的 `SUPPORT_ITEM_ICON` 为 `false` 来将其更改为 `带封面图样式`。
