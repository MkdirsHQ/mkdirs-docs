...
5. 获取 API 令牌
    ...
    进入项目 API 设置，点击 `Add API token` 按钮，创建一个新令牌，并将其设置到 `.env` 文件中。
...
6. 配置 CORS 来源
    ...
    进入项目 API 设置，点击 `Add CORS origin` 按钮，添加您的域名（例如 **`https://your-domain.com`**）或 **`http://localhost:3000`** 用于本地开发。

    它将用于允许您通过 `/studio` 路径访问 Sanity Studio，例如 `https://your-domain.com/studio` 或 `http://localhost:3000/studio`。

    <Aside type="caution">
        确保您将域名的来源添加到 CORS 来源中，否则您将无法访问 Sanity Studio，建议将 `http://localhost:3000` 和 `https://your-domain.com` 都添加到 CORS 来源中。
    </Aside>
...
{/* https://www.youtube.com/embed/HaoYpSv5R1o?list=PLVBbrIi208W-yprj2E6oXu-yxB3Ou522u */}
